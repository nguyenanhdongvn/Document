...
- Enable HTTPS cho gitlab (REF: https://blog.programster.org/dockerized-gitlab-configure-ssl)
    ...
    scp -P 23 /home/dong/ssl/dongna_app.crt sysadmin@gitlab:/home/sysadmin/ssl/server.crt
    scp -P 23 /home/dong/ssl/dongna_app.key sysadmin@gitlab:/home/sysadmin/ssl/server.key
    ```
    ...
    sudo mv /home/sysadmin/ssl/server.* $GITLAB_HOME/config/ssl/gitlab.dongna.com/
    ```
    ...
    nginx['ssl_certificate_key'] = "/etc/gitlab/ssl/gitlab.dongna.com/server.key"       # /etc/gitlab/ssl is the path in docker container
    ...
...
- Truy cập https://gitlab.dongna.com/ và đăng nhập bằng user `root`
...
- Cấu hình nginx
```
cat <<EOF > /etc/nginx/conf.d/jenkins.dongna.com.conf
upstream jenkins{
...
server{
    listen      80;
    server_name jenkins.dongna.com;
    return 301  https://jenkins.dongna.com$request_uri;
}
...
server {
    ...
    ssl_certificate /home/sysadmin/ssl/dongna_app.crt;
    ssl_certificate_key /home/sysadmin/ssl/dongna_app.key;
...
EOF
```
- Kiểm tra syntax và reload config nginx
```
sudo nginx -t
